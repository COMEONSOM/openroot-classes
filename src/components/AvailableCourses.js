...
import msmeLogo from "../assets/msme-logo.png"; // MSME CERTIFICATION LOGO
...
export default function AvailableCourses() {
  ...
  // ---------- PAYMENT: after details modal submit ----------
  ...
  // ---------- PDF: nicely formatted invoice with logo ----------
  const handleDownloadReceipt = async () => {
    try {
      ...
      // MSME logo centered at bottom
      const msmeSize = 60; // adjust 60–90 as you like
      const msmeX = (pageWidth - msmeSize) / 2;
      const msmeY = footerY - msmeSize - 8;

      pdf.addImage(msmeLogo, "PNG", msmeX, msmeY, msmeSize, msmeSize);

      pdf.save(
        `Openroot_Invoice_${paymentMeta?.paymentId || "payment"}.pdf`
      );
    } catch (err) {
      ...
      <AnimatePresence>
        {viewState === "success" && selectedCourse && (
          ...
          >
            ...
            <p>
              You successfully enrolled in <strong>{selectedCourse.name}</strong>
            </p>
